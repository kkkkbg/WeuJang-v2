  ...
  try {
    ...
    fetch(`/api/cards?note_id=${noteId}&page=${page}&sort=${sort}`)
      ...
      .then(data => {
        ...
        <p id='spanTextRigth_${cards[i].card_id}' class='answer spanTextRigths'>${cards[i].answer}</p>
...
document.addEventListener('click', function (e) {
  ...
  if (e.target.classList.contains('hint-btn')) {
    ...
    popup.style.width = `${rect.width - 35}px`;
...
async function scrollToSticker(cardId) {
  let targetNote = document.querySelector(`.note[data-index="${cardId}"]`);
  let maxTries = 10;
  let tries = 0;
  ...
  // 3. 전달받은 index에 해당하는 .note를 찾아 스크롤
  if (targetNote) {
